<ul class="nav nav-pills nav-tabs mb-3" id="pills-tab" role="tablist">
    @if (isset($commission))
        @foreach ($commission as $key => $value)
            <li class="nav-item">
                <a class="nav-link {{ $key == 'mobile' ? 'active' : '' }}" id="pills-home-tab" data-bs-toggle="pill"
                    href="#{{ $key }}" role="tab" aria-controls="pills-home"
                    aria-selected="true">{{ ucfirst($value['label']) }}</a>
            </li>
        @endforeach
    @endif
</ul>
<div class="tab-content" id="pills-tabContent-2">
    ...
    @if (isset($mydata['schememanager']) && isset($commission) && $mydata['schememanager']->value == 'admin')
        @foreach ($commission as $key => $value)
            <div class="tab-pane fade show {{ $key == 'mobile' ? 'active' : '' }}" id="{{ $key }}"
