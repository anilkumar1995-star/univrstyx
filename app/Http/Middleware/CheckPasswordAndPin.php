	...
	public function handle($request, Closure $next, $type)
	{
		if ($type == "password") {
			...
			$validated = \Validator::make(
				$request->all(),
				[
					'pin' => 'required|numeric|digits_between:4,5|not_in:' . implode(',', $arrayValue),
				],
				[
		...
		if ($validated->fails())
			...
			// 		$msg[$key]=$val[0];
			// 	}
			return response()->json(["statuscode" => "ERR", "message" => $validated->errors()->first(), "status" => $validated->errors()->first()], 422);
		// }
