...
class ResourceController extends Controller
{
    public function index($type)
    {
        switch ($type) {
            case 'scheme':
                ...
                $data['allBillPayOperator'] = self::getBillpaymentProviders();
                ...
                $data['ebillOperator'] = [];// Provider::where('type', 'electricity')->where('status', "1")->get(); //->where('mode','online')
                $data['ebillOperatorOffiline'] = [];// Provider::where('type', 'electricity')->where('status', "1")->get(); //->where('mode','offline')
            ...
            case 'commission':
                ...
                $arr = self::getBillpaymentProvidersSlugs();
                $data = [];
                ...
                foreach (@$product as $key) {
                    ...
                    if (!in_array((string) $key['service_slug'], $arr)) {
                        ...
                        // $data['commission'][$key['service_slug']]['details'] = $getData->where('slab', $key['service_slug'])->get();
                        // if ($data['commission'][$key['service_slug']]['details']->isNotEmpty()) {
                        //     $data['commission'][$key['service_slug']]['details'][0]['provider'] = ['name' => $key['service_name']];
                        // }
                        // $data['commission'][$key['service_slug']]['label'] = "Bill Payments";
    ...
    public function update(Request $post)
    {
        ...
        switch ($post->actiontype) {
            ...
            case 'company':
                ...
                //     $filename = 'logo' . $post->id . "." . $post->file('file')->guessExtension();
                //     $post->file('file')->move(public_path('logos/'), $filename);
                //     $post['logo'] = $filename;
                // }
                ...
                if ($post->hasFile('file')) {
                    ...
                    $ImageUpload = ImageHelper::imageUploadHelper('logos', $file);
                    if ($ImageUpload['status']) {
                        $post['logo'] = $ImageUpload['data']['target_file'];
                    } else {
    ...
    // public function getBillpaymentProviders()
    // {
    //     $getBillpaymentId = DB::table('gbl_service_list')->where('service_slug', 'billpayments')->first();
    //     $data = [];
    //     if ($getBillpaymentId) {
    //         $getBillServices = DB::table('gbl_service_list')->where('service_parent_id', $getBillpaymentId->id)->where('is_commission', "1")->orderBy('service_slug')->get();
    //     }
    //     return $getBillServices;

    // }


    public static function getBillpaymentProvidersSlugs()
    {
        $getBillpaymentId = DB::table('gbl_service_list')->where('service_slug', 'billpayments')->first();
        $data = [];
        if ($getBillpaymentId) {
            $getBillServices = DB::table('gbl_service_list')->where('service_parent_id', $getBillpaymentId->id)->where('is_commission', "1")->orderBy('service_slug')->get(['service_slug']);
            foreach ($getBillServices as $key => $value) {
                $data[] = (string) $value->service_slug;
            }
        }
        return ($data);
